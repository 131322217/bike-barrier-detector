...
/* ===== 設定 ===== */ 
const THRESHOLD = 27;        // 段差判定 全体の変化量
const Z_THRESHOLD = 10;      // 段差判定 Z軸単体のしきい値
const CURVE_THRESHOLD = 15;  // カーブ判定 XY軸のしきい値
...
function handleMotion(e){ 
  ...
  if(prevAcc){ 
    ...
    /* ===== 段差判定 ===== */
    if (diff > THRESHOLD && dz > Z_THRESHOLD && dz > dx && dz > dy) { 
      // 距離フィルタ 
    ...
    /* ===== カーブ判定 ===== */
    else if ((dx > CURVE_THRESHOLD || dy > CURVE_THRESHOLD) && dz < Z_THRESHOLD) {
      sample.isEvent = true;
      sample.type = "curve";
      const context = recentSamples.slice(-PRE_N);
      saveEvent(context);
      ...
      L.marker([sample.lat,sample.lng],{icon})
        .addTo(map)
        .bindPopup(`カーブ検出<br>diff=${diff.toFixed(2)}<br>dx=${dx.toFixed(2)}<br>dy=${dy.toFixed(2)}`);

      eventMarkers.push({lat:sample.lat,lng:sample.lng});
      logUI(`カーブ検出 diff=${diff.toFixed(2)}`);
    }
  }
